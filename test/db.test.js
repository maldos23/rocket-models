...
describe("initDatabase", () => {
  ...
  it("should throw an error if an invalid variant is provided", async () => {
    try {
      await rocket.db.initDatabase("mongodb://localhost:27017/myapp", {
        variant: "invalid",
      });
    } catch (err) {
  ...
  it("should call initPostgress if variant is postgres", async () => {
    const initPostgressStub = sinon
      .stub(rocket.db, "initDatabase")
      .returns(Promise.resolve());
    await rocket.db.initDatabase("postgresql://localhost:5432/myapp", {
      variant: "postgres",
    });
